...
All the above entities live in nested namespaces that can be used to organize them hierarchically.
Namespaces can be used for selection and permissions.

Another way to organize entities is to assign them to any number of "groups". Groups are themselves
stored and namespaced entities with their own metadata. They can be used to apply policies for both
data and behavior.

Floria data is designed to be portable and communicable. Entities can be converted to and from
CBOR, MessagePack, JSON, and YAML formats.
