...
Floria topologies are graphs of nested nodes (vertexes) with relationships (edges) between them. Both nodes and relationships are first-class citizens with custom data and metadata.

Nodes can represent software or hardware components at any level from infrastructure to application, or logical configurations that exist purely as data (and metadata). Relationships can represent actual connections, such as network routes, ports, and secure channels, as well as logical dependencies. If nodes are the bones and muscles, then relationships are the connective tissue.
...
All the above entities live in nested namespaces that can be used to organize them hierarchically. Namespaces can be used for selection and permissions.

Another way to organize entities is to assign them to any number of "groups". Groups are themselves entities, stored in namespaces. They can be used to apply policies and type information for both data and behavior.
...
Though Wasm plugins get preferential support in Floria, Wasm is not a requirement. Plugins can be implemented in anything that can be executed on your cloud hardware.
...
Named after Floria Tosca, the protagonist of Victorien Sardou's play, [*La Tosca*](https://en.wikipedia.org/wiki/La_Tosca). She was written for and first portrayed by actress Sarah Bernhardt.
...
* [Apache License, Version 2.0](https://github.com/tliron/rust-floria/blob/main/LICENSE-APACHE)
* [MIT license](https://github.com/tliron/rust-floria/blob/main/LICENSE-MIT)
