...
impl NodeTemplate {
    ...
    /// To [Value].
    pub fn to_value<'own, StoreT>(&self, store: &'own StoreT) -> Result<Value, StoreError>
    where
        StoreT: StoreClient,
    {
        let mut map = BTreeMap::new();

        self.template.to_value(&mut map, store)?;
        ...
        if !self.contained_node_template_ids.is_empty() {
            ...
            for contained_node_template_id in &self.contained_node_template_ids {
                match store.get_node_template(contained_node_template_id)? {
                    Some(node_template) => contained_node_templates.push(node_template.to_value(store)?),
                    None => {}
                }
            }
        ...
        if !self.outgoing_relationship_template_ids.is_empty() {
            let mut outgoing_relationship_templates = Vec::with_capacity(self.outgoing_relationship_template_ids.len());
            for outgoing_relationship_template_id in &self.outgoing_relationship_template_ids {
                match store.get_relationship_template(outgoing_relationship_template_id)? {
                    Some(relationship_template) => {
                        outgoing_relationship_templates.push(relationship_template.to_value(store)?)
                    }
                    None => {}
                }
            }
    ...
    StoreT: StoreClient,
{
        ...
        WriteT: io::Write,
    {
        ...
        context.theme.write_meta(writer, "contained_node_templates")?;
        ...
        if self.node_template.contained_node_template_ids.is_empty() {
            ...
            for (node_template_id, last) in IterateWithLast::new(&self.node_template.contained_node_template_ids) {
                item_context.indent_into_thick_branch(writer, last)?;
                match self
                    .store
                    ...
                    .map_err(|e| io::Error::new(io::ErrorKind::Other, e))?
                {
                    Some(node_template) => {
                        node_template
                            ...
                            .write_debug_for(writer, &item_context.child().increase_indentation_thick_branch(last))?;
                    }
        ...
        context.theme.write_meta(writer, "outgoing_relationship_templates")?;
        context.theme.write_delimiter(writer, ":")?;
        ...
        if self.node_template.outgoing_relationship_template_ids.is_empty() {
            ...
            let item_context = context.child().increase_indentation_branch(true);
