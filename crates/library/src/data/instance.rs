...
impl Instance {
        ...
        StoreT: StoreClient,
    {
        ...
        map.insert("id".into(), self.id.id.clone().into());
        if let Some(origin_template_id) = &self.origin_template_id {
            map.insert("origin_template_id".into(), origin_template_id.id.clone().into());
        }
        ...
        if !self.type_ids.is_empty() {
            if embedded {
                ...
                let type_ids: Vec<Value> = self.type_ids.iter().map(|i| i.id.clone().into()).collect();
