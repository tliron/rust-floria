        ...
        StoreT: StoreClient,
    {
        ...
        if !embedded {
            if let Some(containing_node_id) = &self.containing_node_id {
                map.insert("containing_node_id".into(), containing_node_id.id.clone().into());
            }
        }
        ...
        if !self.contained_node_ids.is_empty() {
            if embedded {
                ...
                let contained_node_ids: Vec<Value> =
                    self.contained_node_ids.iter().map(|i| i.id.clone().into()).collect();
        ...
        if !self.outgoing_relationship_ids.is_empty() {
            if embedded {
                ...
                let outgoing_relationship_ids: Vec<Value> =
                    self.outgoing_relationship_ids.iter().map(|i| i.id.clone().into()).collect();
        ...
        if !embedded && !self.incoming_relationship_ids.is_empty() {
            let incoming_relationship_ids: Vec<Value> =
                self.incoming_relationship_ids.iter().map(|i| i.id.clone().into()).collect();
    ...
    StoreT: StoreClient,
{
        ...
        WriteT: io::Write,
    {
        ...
        context.theme.write_meta(writer, "contained_nodes")?;
        ...
        context.theme.write_meta(writer, "outgoing_relationships")?;
        context.theme.write_delimiter(writer, ":")?;
