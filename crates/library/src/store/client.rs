...
pub trait StoreClient: Clone + Send {
    ...
    fn get_entity_as_value(&self, id: &ID) -> Result<Option<Value>, StoreError> {
        let value_result = match id.kind {
            Kind::Group => self.get_group(&id)?.map(|g| Ok(g.to_value())),
            Kind::NodeTemplate => self.get_node_template(&id)?.map(|n| n.to_value(false, self)),
            Kind::RelationshipTemplate => self.get_relationship_template(&id)?.map(|r| r.to_value(false, self)),
            Kind::Node => self.get_node(&id)?.map(|n| n.to_value(false, self)),
            Kind::Relationship => self.get_relationship(&id)?.map(|r| r.to_value(false, self)),
        };
