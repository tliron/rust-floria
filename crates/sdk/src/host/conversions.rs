    ...
    fn from(value: Value) -> Self {
        match value {
            ...
            Value::NestedList(nested_list) => {
                let list: Vec<Self> = nested_list.get().into_iter().map(|value| value.into()).collect();
                list.into()
            }
...
impl From<data::Value> for Value {
    fn from(value: data::Value) -> Self {
        match value {
            ...
            data::Value::Blob(blob) => Self::Blob(blob),

            data::Value::NestedList(nested_list) => {
                let vector: Vec<Value> = nested_list.to_list().inner.iter().map(|value| value.clone().into()).collect();
                Self::NestedList(NestedList::new(vector))
            }

            data::Value::NestedMap(nested_map) => {
                let key_value_pairs: Vec<(Value, Value)> = nested_map
                    .to_map()
                    .inner
                    .iter()
