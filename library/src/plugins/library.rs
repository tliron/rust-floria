...
/// Plugin library.
pub struct Library<'env, StoreT>
where
    StoreT: 'static + Store,
{
    /// Environment.
    pub environment: &'env Environment<StoreT>,

    /// Dispatch.
    pub dispatch: FastHashMap<String, DispatchPlugin<StoreT>>,
}

impl<'env, StoreT> Library<'env, StoreT>
where
    StoreT: Store,
{
    /// Constructor.
    pub fn new(environment: &'env Environment<StoreT>) -> Self {
