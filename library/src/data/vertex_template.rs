...
impl VertexTemplate {
    ...
    {
        let vertex_id = self.instantiate_vertexes(directory, containing_vertex_id, library, plugin_name, errors)?;
        let mut vertex = library
            .environment
            .store
            .get_vertex(&vertex_id)?
            .ok_or_else(|| super::super::StoreError::ID(vertex_id.to_string()))?;

        vertex.update(library, plugin_name, &library.environment.store, errors)?;
        vertex.validate(library, plugin_name, &library.environment.store, errors)?;

        vertex.instantiate_edges(directory, library, plugin_name, errors)?;
        ...
        ErrorRecipientT: kutil::std::error::ErrorRecipient<super::super::FloriaError>,
    {
        let mut vertex = super::vertex::Vertex {
            instance: self.template.instantiate(Kind::Vertex, directory, &library.environment.store)?,
        ...
        for contained_vertex_template_id in &self.contained_vertex_template_ids {
            match library.environment.store.get_vertex_template(contained_vertex_template_id)? {
        ...
        library.environment.store.add_vertex(vertex)?;
