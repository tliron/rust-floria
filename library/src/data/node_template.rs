use super::{super::store::*, depict::*, id::*, kind::*, scope::*, template::*};
...
impl NodeTemplate {
    /// Constructor.
    pub fn new<StoreT>(scope: Scope, store: &StoreT) -> Result<Self, StoreError>
    where
        StoreT: Store,
    {
        let mut id = ID::new(Kind::NodeTemplate, scope);
        store.create_id(&mut id)?;
        Ok(Self::new_with(id, None))
    }

    /// Constructor.
    pub fn new_for(scope: Scope, id: ByteString, containing_node_template_id: Option<ID>) -> Self {
        Self::new_with(ID::new_for(Kind::NodeTemplate, scope, id), containing_node_template_id)
    }
    ...
    {
        let node_id = self.instantiate_nodes(scope, containing_node_id, library, plugin_name, errors)?;
        let mut node = library
            .environment
            .store
            .get_node(&node_id)?
            .ok_or_else(|| super::super::StoreError::ID(node_id.to_string()))?;
        node.instantiate_relationships(scope, library, plugin_name, errors)?;
        ...
        ErrorRecipientT: kutil::std::error::ErrorRecipient<super::super::FloriaError>,
    {
        let mut node = super::node::Node {
            instance: self.template.instantiate(Kind::Node, scope, &library.environment.store)?,
            containing_node_id,
