use super::{super::store::*, depict::*, id::*, instance::*, kind::*, prefix::*};
...
impl Node {
    /// Constructor.
    pub fn new<StoreT>(prefix: Prefix, origin_template_id: ID, store: &StoreT) -> Result<Self, StoreError>
    where
        StoreT: Store,
    {
        let mut id = ID::new(Kind::Node, prefix);
        store.create_id(&mut id)?;
        Ok(Self::new_with(id, Some(origin_template_id)))
    }

    /// Constructor.
    pub fn new_for(prefix: Prefix, id: ByteString, origin_template_id: Option<ID>) -> Self {
        Self::new_with(ID::new_for(Kind::Node, prefix, id), origin_template_id)
    }
        ...
        ErrorRecipientT: kutil::std::error::ErrorRecipient<super::super::FloriaError>,
    {
        for contained_node_id in &self.contained_node_ids {
            match library.environment.store.get_node(contained_node_id)? {
                Some(contained_node) => {
                    contained_node.instantiate_relationships(prefix, library, plugin_name, errors)?;
                }
