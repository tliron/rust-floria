...
pub fn write_debug_metadata<WriteT, AnnotatedT>(
    metadata: &Metadata<AnnotatedT>,
    last: bool,
    writer: &mut WriteT,
...
pub fn write_debug_properties<'own, StoreT, WriteT, AnnotatedT>(
    name: &str,
    properties: &BTreeMap<ByteString, Property<AnnotatedT>>,
    store: &'own StoreT,
    last: bool,
    writer: &mut WriteT,
    context: &DebugContext,
...
/// Write debug groups.
pub fn write_debug_groups<WriteT, StoreT, AnnotatedT>(
    group_ids: &Vec<ID>,
    store: &StoreT,
    writer: &mut WriteT,
    context: &DebugContext,
    ...
    utils::write_debug_field("groups", false, writer, context, |writer, context| -> io::Result<()> {
        if group_ids.is_empty() {
            ...
            for (group_id, last) in IterateWithLast::new(group_ids) {
                ...
                match store.get_group(group_id).map_err(io::Error::other)? {
                    Some(group) => {
                        group
                            .to_debuggable(store)
