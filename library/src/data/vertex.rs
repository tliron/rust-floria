...
impl Vertex {
        ...
        ErrorRecipientT: kutil::std::error::ErrorRecipient<super::super::FloriaError>,
    {
        ...
        store.add_vertex(self.clone())?;

        for vertex_id in &self.contained_vertex_ids {
            if let Some(mut vertex) = store.get_vertex(vertex_id)? {
                vertex.update(library, plugin_name, store, errors)?;
            }
        }
    ...
    where
        ...
        ErrorRecipientT: kutil::std::error::ErrorRecipient<super::super::FloriaError>,
    {
        self.instance.validate(library, plugin_name, errors)?;
        store.add_vertex(self.clone())?;

        for vertex_id in &self.contained_vertex_ids {
            if let Some(mut vertex) = store.get_vertex(vertex_id)? {
                vertex.validate(library, plugin_name, store, errors)?;
            }
        }
        ...
        ErrorRecipientT: kutil::std::error::ErrorRecipient<super::super::FloriaError>,
    {
        ...
        for contained_vertex_id in &self.contained_vertex_ids {
            match library.environment.store.get_vertex(contained_vertex_id)? {
                Some(contained_vertex) => {
                    contained_vertex.instantiate_edges(directory, library, plugin_name, errors)?;
                }
        ...
        match &vertex.instance.origin_template_id {
            Some(origin_template_id) => match library.environment.store.get_vertex_template(origin_template_id)? {
                Some(vertex_template) => {
                    for outgoing_edge_template_id in &vertex_template.outgoing_edge_template_ids {
                        match library.environment.store.get_edge_template(outgoing_edge_template_id)? {
