...
///
/// Note that if an implementation supports [Clone], then the clone should have access to the sane
/// state. In other words, this trait can be understood as representing a client of the store
...
pub trait Store {
    ...
    /// Get nodes.
    fn get_nodes(&self, prefixes: Option<Vec<Prefix>>) -> Result<Vec<Node>, StoreError>;
