...
#[derive(Clone)]
pub struct InMemoryStoreClient<AnnotatedT> {
    /// Implementation.
    pub implementation: Arc<InMemoryStoreImplementation<AnnotatedT>>,
}

impl<AnnotatedT> InMemoryStoreClient<AnnotatedT> {
    /// Constructor.
    pub fn new(store: Arc<InMemoryStoreImplementation<AnnotatedT>>) -> Self {
        Self { implementation: store }
    }
}

impl<AnnotatedT> Store<AnnotatedT> for InMemoryStoreClient<AnnotatedT>
where
    AnnotatedT: Clone + Send + Sync,
{
    ...
    fn get_nodes(&self, _namespaces: Option<Vec<Namespace>>) -> Result<Vec<Node<AnnotatedT>>, StoreError> {
